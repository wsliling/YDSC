	...
	<view class="pagehome">
		<view class="homeBackground p_re">
			...
			<view class="btn flex-center" v-if="myType==1" @click="changebanner">
				<text class="iconfont icon-fabu mr1 fz16"></text>设置封面
			</view>
		</view>
		<view class="userInfo">
			...
			<view class="fans">
				<view class="fansNum">
					<view @click="toMylink('/pages/member/follow/follow?type=1')">
						<text class="num">{{Info.Fans>9999?(Info.Fans/9999).toFixed(1)+'W':Info.Fans}}</text><text>粉丝</text>
					</view>
					<view @click="toMylink('/pages/member/follow/follow?type=0')"><text class="num">{{Info.Follow}}</text> <text>关注</text></view>
				</view>
				<block v-if="myType==0">
				...
				<view class="fansBtn" v-else>
					<view class="btn" @click="tolink('/pages/member/editinfo/editinfo')"><text class="iconfont icon-fankui mr1"></text>编辑资料</view>
				</view>
			</view>
		</view>
		<view class="user">
			...
			<view class="userSign uni-ellipsis2" v-if="Info.Introduction">
				{{Info.Introduction}}
			</view>
		...
		<view style="overflow-x: hidden;">
			...
			<view class="tabCon tab-img flex flexWrap" v-if="tabIndex==1">
				...
				<view class="item" @longtap="longtap(item,index,1)">
					<image :src="item.Logo||'http://via.placeholder.com/400x400'" mode="aspectFill" @click.stop="tolink('/pages/hwDetails/hwDetails?id='+item.Id+'&MemberId='+item.MemberId)"></image>
					<view class="delbox flex-center" v-if="item.isDel" @click.stop="hidedel(item,index)">
						<text class="delbtn" @click.stop="delData(item.Id,index)">删除</text>
					</view>
				</view>
			...
			<view class="tabCon tab-find" v-if="tabIndex==2&&hasData">
				...
				<view class="media-item flex" @click="tolink('/pages/personal/replylist/replylist?id='+item.Id)">
					<view class="media-hd">
						<text class="day">{{item.day}}</text><text>/{{item.month}}月</text>
					</view>
		...
		<uni-popup v-if="myType==1" ref="bannerWin" type="center">
			<view class="uni-modal-banner" @click="hidePopup">
				...
				<view class="btn btn_red" @click.stop="chooseImg">
					更换封面
				</view>
				<view class="btn" @click.stop="hidePopup">
					取消
				</view>
			</view>
		</uni-popup>
