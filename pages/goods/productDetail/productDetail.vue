<template>
	<view v-if="hasData">
		<view class="productBanner">
			...
			<!-- #endif -->
			<video v-if="showVideo" :src="proInfo.Video" :class="['video', isfixed ? 'top' : '']" id="proVideo" @play="playVideo" @pause="pauseVideo"></video>
			<swiper class="swiper" :indicator-dots="false" :autoplay="false" :interval="5000" :duration="500" @change="changeBanner">
				<swiper-item v-if="proInfo.Video" :show-fullscreen-btn="false" :show-play-btn="false" show-mute-btn :enable-progress-gesture="false">
					...
					<view class="swiper-item"><image class="img" :src="proInfo.PicData[0].PicUrl" mode="aspectFill"></image></view>
				</swiper-item>
				<swiper-item v-for="(item, index) in proInfo.PicData" :key="index">
					<view class="swiper-item"><image class="img" :src="item.PicUrl" mode="aspectFill" @click.stop="previewImg(proInfo.imgArr, index)"></image></view>
				</swiper-item>
			</swiper>
		...
		<view :class="['red_price flash flex-between pd15', starTimetype == 0 ? 'bg-green' : '']" v-if="isLimint == 1">
			...
			<view class="gr_num flex-column">
				<text class="txt">
					<text class="iconfont icon-shangou"></text>
					限时秒杀
				</text>
				<view class="time flex-end">
					<text class="timetxt">{{ starTimetype == 1 ? '距离结束' : '距离开始' }}</text>
					<view class="countDown">
						<text>{{ timeStr[1] || '00' }}</text>
						:
						<text>{{ timeStr[2] || '00' }}</text>
						:
						<text>{{ timeStr[3] || '00' }}</text>
					</view>
				</view>
			</view>
		</view>
		<view class="uni-bg-white pd15 uni-mb10 pdt15">
			...
			<view class="site">
				...
				<view class="desc uni-ellipsis2">{{ proInfo.Synopsis ? proInfo.Synopsis : '' }}</view>
			</view>
			<view class="discount flex-between">
				<view class="txt payment">已售{{ proInfo.SalesVolume ? proInfo.SalesVolume : 0 }}</view>
				<view class="txt addr" v-if="proInfo.ShowArea">{{ proInfo.ShowArea }}</view>
				<view class="txt" v-else>{{ proInfo.IsPinkage ? '运费：' + proInfo.Freight + '元' : '包邮' }}</view>
			</view>
		</view>
		<view class="shipments uni-bg-white uni-mb10" v-if="proInfo.IsAloneBuy == 0">
			...
			<view class="pick flex-between" v-if="proInfo.ParameterJson !== '{}' && proInfo.ParameterJson !== ''" @click="showWin('参数')">
				...
				<view class="flex1">{{ attrArr0 }}</view>
				<view class="iconfont icon-arrow_r"></view>
			</view>
			...
			<view class="pick flex-between needknow" v-if="isServiceInfo" @click="showWin('服务')">
				...
				<view class="flex-start flex1 flexWrap">
					<view class="drawinfo" v-if="index < 3" v-for="(item, index) in proInfo.ServiceInfo" :key="index">
						<text class="iconfont icon-gou c_theme"></text>
						{{ item.Name }}
					</view>
				</view>
				<view class="iconfont icon-arrow_r"></view>
		...
		<block v-if="GroupId > 0">
			...
			<view class="swiper_bb uni-bg-white uni-mb10" v-if="recordList.length > 0">
				<view class="Yi-hd flex-between uni-mb10"><view class="title">他们都在拼，可直接参团</view></view>
				<swiper
					:class="recordList.length > 1 ? 'swiper_pt swiper_height ' : 'swiper_pt'"
					vertical="false"
					:display-multiple-items="displayMultiple"
					:circular="true"
					:autoplay="true"
					interval="5000"
					:duration="600"
					...
					<swiper-item class="swiper_item_box" v-for="(item, sindex) in recordList" :key="sindex">
						<view class="item_box_left flex-start flex1">
							...
							<view class="time-box flex1">
								...
								<view class="time">
									还差
									<text class="c_theme">{{ item.MaxPeople - item.CurrentPeople }}</text>
									人成团，剩余
									<text class="c_theme">{{ item.EndTime[0] }}:{{ item.EndTime[1] }}:{{ item.EndTime[2] }}</text>
									结束
								</view>
							</view>
						</view>
		...
		<view class="comment uni-bg-white uni-mb10">
			<view class="Yi-hd flex-between">
				<view class="title">
					商品评价
					<text class="c_999 font26">({{ commentCount }})</text>
				</view>
			...
			<block v-if="hasComment">
				<view class="comment-item" v-for="(item, index) in CommentList" :key="index">
					...
					<view class="flex flexWrap imgs" v-if="item.PicData">
						<block v-for="(i, e) in item.imgArr" :key="e"><image v-if="e < 3" :src="i" mode="" class="shop_pic" @click="previewImg(item.imgArr, i)"></image></block>
					</view>
					<view class="time">{{ item.AddTime }}</view>
				</view>
			</block>
			<view class="uni-comment" v-else><view class="uni-center" style="padding: 20upx; color: #999;">暂无评论</view></view>
		</view>
		<!-- 店铺 -->
		...
		<view class="uni-bg-white">
			...
			<view class="detailhtml" v-if="proInfo.ContentDetail"><uParse :content="proInfo.ContentDetail" /></view>
		</view>
		<!-- 详情底部 -->
		...
		<view class="foot-fiexd">
			<view class="dd-foot">
				...
				<view class="foot-item" style="position: relative;">
					<!-- #ifndef MP-WEIXIN -->
					<view class="bd" @click="tolink('/pages/member/kefu/kefu?qqnum=996889692')">
						<view class="iconfont icon-kefu"></view>
						客服
					</view>
					<!-- #endif -->
					<!-- #ifdef MP-WEIXIN -->
					<button class="bd" open-type="contact" style="border:none !important;padding:0;background: #ffffff;line-height:1.9">
						<view class="iconfont icon-kefu"></view>
						客服
					</button>
					<!-- #endif -->
				...
				<view class="foot-item">
					<view class="bd">
						<view class="iconfont" :class="[IsCollect ? 'icon-collect' : 'icon-collect1']" @click="collect"></view>
						收藏
					</view>
				</view>
				<view class="foot-item" style="margin-right: 20upx;" @click="tolink('/pages/goods/cart/cart')">
					<view class="bd">
						<view class="iconfont icon-tianchongxing- p_re">
				...
				<view class="foot-item foot-item-btns" v-if="proInfo.IsAloneBuy == 0">
					<view class="btn btn_1 flex no" @click="showSku(3)" v-if="proInfo.IsAloneBuy == 0 && GroupId == 0 && islive == 0"><view class="txt">加入购物车</view></view>
				...
				<view class="foot-item foot-item-btns" v-else>
					...
					<block v-else>
						<view class="btn btn_1 flex" style="background-color: #C9C9C9;"><view class="txt" style="font-size: 30upx;">已售罄</view></view>
					</block>
				</view>
			</view>
		</view>
						...
						<view class="coupon-item uni-mb10" v-for="(item, index) in CouponList" :key="index">
							<view class="flex-between">
								<view class="couponleft flex1">
									<view class="title">
										<view class="price" v-if="item.DiscountType == '1'">
											<text>￥</text>
											<text class="num">{{ item.Denomination }}</text>
										</view>
										<view class="price" v-if="item.DiscountType == '2'">
											<text class="num">{{ (item.Denomination * 100) / 10 }}</text>
											<text>折</text>
										</view>
					...
					<view class="attrlist uni-mb10">
						<view class="uni-list-cell" v-for="(item, index) in proInfo.ServiceInfo" :key="index">
							<view class="uni-list-cell-navigate flexWrap">
								<view>
									<text class="iconfont icon-gou c_theme mr1"></text>
									{{ item.Name }}
								</view>
		...
		<uni-popup ref="ruleWin" type="center">
			<view class="uni-modal-rule">
				<view class="inner uni-bg-white b_radius">
					...
					<view class="bottom-content">
						<view class="h3">更新时间：</view>
						<view class="c_999">不定期更新，限量放送,售完即止</view>
						<view class="h3">参与步骤：</view>
						<view class="c_999">
							1.在拼团商品活动页面中选择商品，点击购买进入支付页
							<br />
		...
		<uni-popup ref="popupShare" type="bottom">
			<nvue-share
				:title="proInfo.Name"
				:imageUrl="proInfo.PicData[0].PicUrl"
				:url="'/pages/goods/productDetail/productDetail?proId=' + proId + '&inCode=' + ReferralCode + '&isLimint=' + isLimint"
			></nvue-share>
		</uni-popup>
		<!-- 悬浮窗 -->
		<!-- #ifndef MP-WEIXIN -->
