		...
		<view class="uni-pd10 p_re">
			<view class="addrbox uni-bg-white b_radius uni-mb10" v-if="(info.IsAloneBuy==0&&info.IsSalesOffice==0&&orderSType==0) || orderSType==1">
				...
				<view class="noaddr" v-else @click="golink('/pages/member/address/address?pagetype=confirm')">
					<view class="icon icon_addr"><image src="http://xcx.yixijiu19.com/static/no_addr2.png" mode="widthFix"></image></view>
				...
				<view class="procontent b_radius uni-mb10" v-for="(item,index) in info.CartList" :key="index">
					...
					<view class="orderbox">
						...
						<view class="orderinfo" v-if="item.UseCouponList.length" @click="openshopCoupon(item.UseCouponList,item.CouponId,index)">
							...
							<view class="orderright">
								<view class="infotxt">{{item.yhPrice?'-￥'+item.yhPrice:'不使用'}}</view>
...
<script>
	...
	export default {
		...
		onLoad: function(e) {
			...
			if(e.GroupRecordId){
				this.GroupRecordId = e.GroupRecordId
			}
			// #endif
		},
		onShow() {
			...
			if(this.$root.$mp.query.GroupRecordId){
				this.GroupRecordId = this.$root.$mp.query.GroupRecordId
				console.log(this.GroupRecordId,"】】】】】")
			}
			// #endif
		...
		methods: {
			...
			async BuyGroup(){
			  ...
			  if(result.code==0){
				  let _this=this;
				  _this.$set(result.data,'SpecText',_this.SpecText)
