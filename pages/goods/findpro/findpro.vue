	...
	<view style="overflow: hidden;">
		...
		<view class="classifytab">
		 	<scroll-view id="tab-bar" v-if="!isshowAll" class="index-swiper-tab" scroll-x :scroll-left="scrollLeft">
		 		<view v-for="(tab,index) in categoryList" :key="index" :class="['item',tabIndex==index ? 'active' : '']" :id="'tabNum'+index" :data-current="index" @click="tapTab(index,tab.Id)" style="width: auto; padding: 0 20upx;">{{tab.Name}}</view>
		 	</scroll-view>
		...
		<view class="p_re">
			...
			<view class="pp3" style="position: relative; z-index: 2;">
				...
				<view class="uni-bg-white b_radius" v-if="noDataIsShow">
...
<style lang="scss" scoped>
	...
	.classifytab{
		...
		/* #endif */
		z-index: 4;
		.index-swiper-tab::before{ display: none;}
		.index-swiper-tab{
			height: 80upx;
			line-height: 80upx;
			padding-left: 20upx;
		...
		.menuIco{
			...
			&::before{
				...
				width: 12upx;
				height: 60upx;
				box-shadow: 0 0 30upx rgba(0,0,0,.3);
				position: absolute;
				top: 50%;
				margin-top: -30upx;
				left: -12upx;
		...
		.showClassify{
			...
			z-index: 9;
			.allhd{ line-height: 80upx; font-size: 30upx;padding: 0 10upx; }
			.btn{
				margin-bottom: 30upx;
			}
		}
