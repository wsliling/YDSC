	...
	<div class="page">
		<div class="weui-cells card__weui-cells">
			<div class="weui-cell" @click.stop="showSelbankCard">
				<img :src="bankLogo || 'https://yd.wtanvxin.com/static/bankcard.png'" class="cardpic" />
				<div class="weui-cell__bd" v-if="hasData">
					<p class="name">{{ bankCardName }}</p>
					<p class="cardnnum">尾号{{ bankCardNo }}储蓄卡</p>
				</div>
				<div class="weui-cell__bd" v-else><p class="name">请选择银行卡</p></div>
		...
		<div class="btns"><div class="btn boxSize bg_ff952e color_fff center" @click="submitWithdraw">确认提现</div></div>
		<view class="mask" v-if="showselbox" @click.stop="showSelbankCard"></view>
		<view class="selbankCard" v-if="showselbox">
			...
			<view class="bankCardList">
				<view class="listItem" v-for="(item, index) in bankList" :key="index" @click="getcardId(item.Id, index)">
					...
					<view class="listRight " :class="bankCardId == item.Id ? 'active' : ''"><text class="iconfont icon-gou1"></text></view>
				</view>
			</view>
			<view class="addBankCard" @click="golink('/pages/Wallet/bankList/bankList')">
				...
				<view><text class="iconfont icon-arrow_r"></text></view>
			</view>
		</view>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		getcardId(id, index) {
			...
			this.bankCardNo = this.bankList[index].BankCardNo.substring(this.bankList[index].BankCardNo.length - 4, this.bankList[index].BankCardNo.length);
			...
			post('Bank/BankList', {
				...
				if (result.code === 0) {
					if (result.data.length > 0) {
						...
						that.bankCardNo = result.data[0].BankCardNo.substring(result.data[0].BankCardNo.length - 4, result.data[0].BankCardNo.length);
					}
				}
			});
		},
		...
		DrawMoneyApply() {
			...
			// }
			post('DrawMoney/memberDrawMoneyApply123', {
				UserId: that.userId,
				Token: that.token,
				Amount: that.amount,
