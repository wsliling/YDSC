	...
	<view>
		...
		<view class="Detailbox uni-mt10" v-if="dataInfo">
			<view class="media-list">
				<view class="tit uni-mb10" v-if="NewsInfo.ContentDetails && NewsInfo.Title">{{ decodeURIComponent(NewsInfo.Title) }}</view>
				...
				<view>
					<view class="media-title detailhtml" v-if="NewsInfo.ContentDetails"><uParse :content="decodeURIComponent(NewsInfo.ContentDetails)" /></view>
		...
		<view class="zanlist uni-bg-white pd15">
			<view class="Yi-hd flex-between"><view class="title">点赞区</view></view>
			<view class="bd flex-between">
				...
				<view class="c_999 flex1" v-if="nolike">还没有人点赞哦</view>
		...
		<view class="comment-list uni-bg-white" v-if="hasData">
			...
			<view class="bd">
				<block v-for="(item, index) in datalist" :key="index"><reply-item :itemData="item" @Sendreplay="Sendreplay"></reply-item></block>
			</view>
		</view>
		...
		<view class="foot-fiexd">
			...
			<view :class="['foot-reply flex', IsShowReplyBox ? 'active' : '']">
				...
				<view class="btn-r">
					<block v-if="IsShowReplyBox"><view :class="['sendBtn', Comment == '' ? 'dis' : '']" @click="Send">发表</view></block>
...
export default {
	...
	methods: {
		...
		/*获取发现详情*/
		async FindNewsInfo() {
			let result = await post('Topic/TopicDynamicInfo', {
				UserId: this.userId,
				Token: this.token,
				DynamicId: this.Findid
			});
			if (result.code === 0) {
		...
		async CommentOperation() {
			...
			if (result.code === 0) {
				...
				this.placeholder = '我也来说~';
				this.PCommentname = '';
				(this.PCommentId = 0), (this.Comment = '');
				setTimeout(() => {
					this.page = 1;
					this.datalist = [];
					this.CommnetList();
