				...
				<block  v-for="(item,index) in datalist" :key="index">
					<div class="avatarhead  boxSize p2 padwids">
						<div class="circlePosition" @click="navigate('homepage/homepage',{id:item.MemberId})">
							...
							<div class="avatarbox">
								<img :src="item.Avatar||'https://yd.wtanvxin.com/static/default.png'" alt="" class="avatar">
							</div>
							<span class="circleNum msgnum" v-if="item.Count>0">{{item.Count }}</span>
						</div>
						<div class="flex flexColumn flex1" @click="navigate('personal/chat/chat',{id:item.MemberId})" >
							<div class="flex">
								<span class="flex1 font32">{{item.NickName}}</span>
								<span class="fz12 c_999">{{item.PubTime}}</span>
							</div>
