			...
			<block v-for="(item, index) in TypeList" :key="index">
				<view class="uni-list-cell" @click="goDetail(item)">
					<view class="uni-media-list">
						<view class="uni-media-list-logo"><image class="img" :src="item.TypeImg" mode="aspectFill"></image></view>
						<view class="uni-media-list-body">
							<view class="uni-media-list-text-top uni-ellipsis">{{ item.MsgName }}</view>
							<view class="uni-media-list-text-bottom uni-ellipsis uni-mt10">{{ decodeURIComponent(item.NoticeTitle) }}</view>
						</view>
