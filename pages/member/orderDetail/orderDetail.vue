<template>
	<view class="ticket pw3" v-if="hasData">
		...
		<view class="or_list">
			...
			<view class="or_item uni-mb10">
				<view class="bg_fff b_radius">
					...
					<view
						class="pp3 flex"
						v-for="(item, index) in info.OrderDetails"
						:key="index"
						@click="goUrl('/pages/goods/productDetail/productDetail?proId=' + item.ProductId)"
					>
						<image mode="aspectFit" :src="item.PicNo" alt="" class="shop mr2"></image>
						<view class="flex1 flex">
							...
							<view class="mr3 flex flexColumn flexAlignEnd" style="margin-left: auto;">
								...
								<text
									class="tui_pill mt2"
									v-else-if="item.IsComment == 1"
									@click.stop="goUrl('/pages/member/addComment/addComment?id=' + info.OrderNumber + '&detailId=' + item.Id)"
								>
									去评价
								</text>
								<text
									class="c_blue fz12"
									v-else
									@click.stop="goUrl('/pages/member/orderTuidetail/orderTuidetail?orderNo=' + info.OrderNumber + '&RefundId=' + item.RefundId)"
								>
		...
		<view class="bg_fff mt2 pp3 bor_tit b_radius order_posi">
			<div class=""><span class="order_title">订单信息</span></div>
			<view class="mt2 order_info font24">
				<!-- #ifdef H5 -->
				<input type="text" class="font20 yy_ma mt1" @focus="blur()" :disabled="disabled" v-model="info.OrderNumber" style="opacity: 0;position: fixed;top: -10000px;" />
				<!-- #endif -->
				<view class="txt">
					订单编号：{{ info.OrderNumber }}
					<text class="copy" @click="cop()">复制</text>
				</view>
		...
		<button
			open-type="contact"
			@contact="btnConcat"
			class="bg_fff pp2 flex-center b_radius mt2 order_posi"
		...
		<view class="flex-end bg_fff mt2 bb_fix btnbox">
			<p
				class="btn btn_gray"
				v-if="info.StatusName == '已发货' || info.StatusName == '已收货'"
				@click="goUrl('/pages/member/logistics/logistics?orderNo=' + info.OrderNumber)"
			>
				查看物流
			</p>
