<template>
	...
	<view>
		...
		<view :style="{height:(89+barHeight)+'px'}"></view>
			...
			<view class="listbox" v-for="(item, index) in datalist" :key="index">
				...
				<view class="drawing flex with-100" @click.stop="tolink('/pages/goods/productDetail/productDetail?proId='+item.ProId,item.Status)">
					<view class="p_re">
						...
						<image class="imgs" :src="item.PicNo||'http://via.placeholder.com/800x800'" mode="aspectFill"></image></view>
					<view class="brace">
						<view class="being uni-ellipsis2">{{ item.Name }}</view>
						<view class="fz12 c_999" v-if="item.SalesVolume">
							{{item.SalesVolume}}人购买
						</view>
						<view class="correct">
							<span class="spanl">¥</span>{{ item.Price }}
						</view>
					</view>
				</view>
			</view>
