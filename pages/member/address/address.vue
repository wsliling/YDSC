<template>
	<view style="padding-bottom:120upx">
		<radio-group class="radio-group" @tap="radioChange" v-if="hasData">
			<view class="list bpr-box" v-for="(item, index) in list" :key="index" @click="choseAddress(index)">
				...
				<view class="ali-c jus-b foot gou flex justifyContentBetween flexAlignCenter">
					<label @click.stop="changeDefault(index)">
						<radio class="radio" color="#FF3333" :checked="item.IsDefault == 1"><text>默认地址</text></radio>
					</label>
					<view class="ali-c flex justifyContentBetween flexAlignCenter">
						<view class="ali-c btt flex flexAlignCenter" @click.stop="editSite(item.Id)">
							<image class="write" src="https://yd.wtanvxin.com/static/write.png" alt=""></image>
							<span>编辑</span>
						</view>
						<view class="ali-c btt flex flexAlignCenter" @click.stop="Delete(item.Id, index)">
							<img class="delete" src="https://yd.wtanvxin.com/static/del.png" alt="" />
							<span>删除</span>
						</view>
					</view>
				</view>
		...
		<p class="nodata_log flex flexColumn flexAlignCenter" v-if="noDataIsShow">
			<image src="https://yd.wtanvxin.com/static/site.png" alt="" class="site_log"></image>
			<span class="cg mt2">还没有收货地址哦~</span>
			<span class="add_new" @tap="toAddSite()">新建地址</span>
		</p>
		<p class="ovedMsg" v-if="isOved && page > 1" style="text-align:center;padding:20upx;font-size:26upx;color:#666;">我也是有底线的</p>
