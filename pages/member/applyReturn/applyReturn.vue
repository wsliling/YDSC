	...
	<view class="sertype">
		...
		<view class="pw3 mt2 bg_fff">
			...
			<view class="mt2">
				...
				<p class="sign" v-show="!showTextArea" @click="showTextArea = true" :class="RefundContent.length > 0 ? 'well_color1' : 'well_color2'">
					{{ RefundContent || '请输入说明详情' }}
				</p>
				<textarea
					class="sign"
					:focus="showTextArea"
					style="padding:10upx 3upx;"
					placeholder="请输入说明详情"
					v-model="RefundContent"
					@focus="showTextArea = true"
					@blur="showTextArea = true"
					v-show="showTextArea"
				></textarea>
			</view>
		</view>
		<view class="mt2 bg_fff pp3" v-if="false">
			...
			<view class="fed_pic flex flexWrap">
				<view v-for="(item, index) in 5" :key="index" class="picbox">
					<image src="https://yd.wtanvxin.com/static/images/icons/add3.png" alt="" class="pic_itim"></image>
				</view>
				<view class="picbox upBtnImg"><image src="https://yd.wtanvxin.com/static/images/icons/add3.png" alt="" class="pic_itim"></image></view>
			</view>
		</view>
	...
	methods: {
		...
		getDetail() {
			post('Order/OrderDetails', {
				...
				this.amount = res.data.OrderDetails[this.indexId].UnitPrice;
			});
		},
		submitSerty() {
			if (this.RefundReasonId == 0) {
				...
				post(url, {
					...
					if (res.code === 0) {
						...
						setTimeout(() => {
							uni.redirectTo({
								url: '/pages/member/orderTuidetail/orderTuidetail?orderNo=' + this.OrderNumber + '&RefundId=' + res.data + '&refundAmount=' + this.amount
							});
							//uni.navigateBack();
						}, 1500);
