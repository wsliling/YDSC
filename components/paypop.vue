<template>
  <view>
    ...
    <div v-if="showPayStatus===2" class="paymask">
      <div class="paytile">
		  <text class="iconfont icon-arrow c_999 leftposi" @click="showPayStatus = 1"></text>
          <text>选择支付方式</text>
      </div>
      <div>
          ...
          <block v-for="(item,index) in payTypeList" :key="index">
            <label class="flex-between payitem" :class="{'disable':integralDisable&&item.code=='integral'}" @click="changes(item)" v-if="item.status">
              <div class="flex-center">
				  <image :src="item.icon" mode="aspectFill" class="payimg"></image>
                {{item.name}}{{integralDisable&&item.code=='integral'?'（积分不足）':''}}
              </div>
              <block v-if="item.code!='integral'||(!integralDisable&&item.code=='integral')">
                <radio name="payType"  :checked="payType.id==item.id" value="0" color="#FF3333"/>
              </block>
            </label>
          </block>
        </radio-group>
...
export default {
  props: {
    ...
    payMode:{
      type:Array,
      default(){return ['wx','alipay','balance']}
    },
    disableIntegral:{
      type:Boolean,
      default:false
  ...
  data() {
    return {
      ...
      password:'',
      forgetPasswordUrl:'/pages/other/ChangepayPwd/ChangepayPwd',//忘记密码跳转url
      payType: {},//选择的支付方式
      payTypeList:[
        {
          name:'微信',
          icon:'http://xcx.yixijiu19.com/static/pay_weixin.png',
          id:0,
          code:'wx',
          status:true,
        },
        ...
        {
          name:'支付宝',
          icon:'http://xcx.yixijiu19.com/static/pay_alipay.png',
          id:2,
          code:'alipay',
          status:true,
        },
        ...
        {
          name:'余额',
          icon:'http://xcx.yixijiu19.com/static/pay_yue.png',
          id:1,
          code:'balance',
          status:true,
        },
        {
          name:'积分',
          icon:'http://xcx.yixijiu19.com/static/pay_jf.png',
          id:3,
          code:'integral',
          status:false,
        },
