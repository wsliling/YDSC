<template>
	<view>
		...
		<view :class="['uni-popup',show?'trans':'']">
					...
					<view class="itembox clear">
						...
						<view class="txtbox">
							...
							<view class="uni-product-price">
								<text class="saleprice"><text class="fz12" v-if="isLimint==1">抢购价</text><text class="fz12">￥</text>{{price}}</text>
				...
				<block v-if="showbtntype==0">
					<view class="bottom-btns">
						<view class="btn addcart" @click="toAddcart">
							加入购物车<text v-if="isLimint==1&&starTimetype==0">(￥{{oprice}})</text>
						</view>
						<view class="btn buynow" v-if="starTimetype==0&&isLimint==1" style="opacity: .6;" @click="remind">
							<view class="txt" style="font-size: 28upx;">即将开始</view>
						</view>
						<view class="btn buynow" v-else @click="gouBuy">
							立即购买
						</view>
					</view>
				</block>
				<view class="bottom-sure" v-if="showbtntype==1" style="line-height: 80upx;" @click="sureSku">{{fromcart?"确定":GroupRecordId?"确定拼团":"确定开团"}}</view>
				<block v-if="showbtntype==2">
					<view class="bottom-sure" v-if="starTimetype==0&&isLimint==1" style="opacity: .6;" @click="remind">
						<view class="txt" style="font-size: 28upx;">即将开始</view>
					</view>
					<view class="bottom-sure" v-else @click="gouBuy">
						{{isLimint==1?"立即抢购":"立即购买"}}
					</view>
				</block>
				<block v-if="showbtntype==3">
					<view class="bottom-sure" @click="toAddcart">
						确定<text v-if="starTimetype==0&&isLimint==1">(￥{{oprice}})</text>
					</view>
				</block>
			</view>
			...
			<view class="popup-ft" v-if="showbtntype==0||showbtntype==2">
				<view class="bottom-btns">
					<view class="btn addcart" @click="toAddcart" v-if="showbtntype==0">
						加入购物车<text v-if="starTimetype==0&&isLimint==1">(￥{{oprice}})</text>
					</view>
					<view class="btn buynow" v-if="starTimetype==0&&isLimint==1" style="opacity: .6;" @click="remind">
						<view class="txt" style="font-size: 28upx;">即将开始</view>
					</view>
	...
	export default {
		...
		methods: {
			hide: function() {
				...
				this.$emit('hidePopup');
				const data = [this.number,this.SpecText,canaddcar,this.price,this.plusprice]
				this.$emit('getsku',data);
			},
			change: function(msg) {
				console.log(msg)
				...
				if (isHide || foreverHide) {
					...
					if (selectedSkuKeyLen === this.skuKey.length) {
						...
						if(this.isLimint==1){
							...
							// this.oprice=selectedSpeList.Price;
						}else{
							if(this.fromPinTuan){
			...
			goodsDetail() {
				if(this.isLimint==1){
					...
					if(this.fromPinTuan){
						...
						this.SpecList=this.proInfo.Sku;
					}else{
			...
			sureSku(){
				if(this.valSubmit()){
					if(this.fromcart){
						...
						this.$emit('getsku',[this.number,this.SpecText,true,this.price]);
						this.$emit('hidePopup');
						if (toLogin()){
							if(this.valSubmit()){
								let money="";
								...
								let objUrl = '/pages/goods/submitOrder/submitOrder?id='+this.proId+'&SpecText='+this.SpecText+'&number='+this.number+'&orderSType=0'+'&GroupId='+this.proInfo.GroupId+'&GroupRecordId='+this.GroupRecordId+"&inCode="+this.inviteCode;
			...
			gouBuy(){
				if (toLogin()){
					if(this.valSubmit()){
						...
						let objUrl = '/pages/goods/submitOrder/submitOrder?id='+this.proId+'&SpecText='+this.SpecText+'&number='+this.number+'&orderSType=0'+'&isLimint='+this.isLimint+"&inCode="+this.inviteCode;
						uni.setStorageSync("addressinfo",'');
						uni.navigateTo({
							url: objUrl
						})
...
.numbox{
	...
	.pop-product .plusprice .icon-plusprice {
		...
		height: 16px;
		width: 33px;
		background: url(http://xcx.yixijiu19.com/static/plus/plus_icon.png) right center no-repeat;
		background-size: 100%;
	}
